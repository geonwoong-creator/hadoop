...
public class AccessLogMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void setup(Mapper<LongWritable, Text, Text, Text>.Context context)
        ...
        //MongoDriver 라이브러리는 제공하짖 않아, 컬렉션 존재 여부 체크 함수를 만들어서 사용해야 함
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        String reqMethod = ""; //GET, POST요청 방법 추출

        if (fields[5].length() > 2) { //일부 데이터가 요청일시 값이 누락된 경우가 있어 요청일시 값이 존재하는지 체크
            reqMethod = fields[5].substring(1); //요철일시 추출
        }
        ...
        pDTO.setIp(ip);
        pDTO.setReqTime(reqTime); //요청시간
        pDTO.setReqMethod(reqMethod); //요청방법
